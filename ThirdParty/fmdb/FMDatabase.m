    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                WPNSLOG(@"%s:%d", __FUNCTION__, __LINE__);
                WPNSLOG(@"Database busy, unable to close");
                return NO;
            }
            ...
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, 0x00)) !=0) {
                    WPNSLOG(@"Closing leaked statement");
                    sqlite3_finalize(pStmt);
                }
            }
        }
        else if (SQLITE_OK != rc) {
            WPNSLOG(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (BOOL)rekey:(NSString*)key {
    ...
    if (rc != SQLITE_OK) {
        WPNSLOG(@"error on rekey: %d", rc);
        WPNSLOG(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    WPNSLOG(@"The FMDatabase %@ is currently in use.", self);
